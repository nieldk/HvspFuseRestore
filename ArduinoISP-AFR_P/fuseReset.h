...
#define HVSP_SDI       PB3      // Serial Data Input on target

#define HVSP_SII_OUT   PORTB
#define HVSP_SII_DDR   DDRB
#define HVSP_SII       PB4      // Serial Instruction Input on target

#define HVSP_SDO_OUT   PORTB
#define HVSP_SDO_IN    PINB
#define HVSP_SDO_DDR   DDRB
#define HVSP_SDO       PB5      // Serial Data Output on target
...
#define HVSP_VCC       PC0      // VCC to target
...
#define LED_DDR        DDRD
#define LED_PIN        PD3      // LED to show result

#define SWITCH_OUT     PORTD
#define SWITCH_DDR     DDRD
#define SWITCH_IN      PIND
#define SWITCH_PIN     PD4      // Push button to start fuse retore 
...
static void chipErase(void) {
  transferByte(0b10000000, 0b01001100);
  transferByte(0b00000000, 0b01100100);
  transferByte(0b00000000, 0b01101100);
}
...
//  LED_DDR   = (1<<LED_PIN);                    //moved to sketch
//  LED_OUT |= (1<<LED_PIN);
  
  /* turn on internal pull-up for push button */
//  SWITCH_DDR &= ~(1<<SWITCH_PIN);                    //moved to sketch
//  SWITCH_OUT |= (1<<SWITCH_PIN);
  
 
//  while(1)                                           //removed, being called from sketch loop()
//  {
    uint8_t device;
